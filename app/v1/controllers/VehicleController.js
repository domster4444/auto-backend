...
export const getVehicle = async (req, res) => {
  try {
    const vehicle = await Vehicle.findById(req.params.id).populate("user_id", { _id: 1, name: 1, phone_number: 1 }).populate("organization_id", { _id: 1, name: 1 });
...
export const changeVerificationStatus = async (req, res) => {
  const vehicle = await Vehicle.findByIdAndUpdate(req.params.id, { is_verified: req.body.is_verified }, { new: true });
  return sendSuccessResponse(res, "Vehicle Update Successful", vehicle);
};
